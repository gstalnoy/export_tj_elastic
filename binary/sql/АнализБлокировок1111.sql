...
FROM   
	MASTER.dbo.sysprocesses pr1(NOLOCK)
...
FROM   
	sys.dm_tran_locks AS tran_locks WITH (NOLOCK)
       ...
       JOIN (
            ...
            ON  RD.[sResource_type] = tran_locks.resource_type AND RD.[id] = tran_locks.resource_associated_entity_id
       OUTER APPLY sys.[dm_exec_sql_text](s.[sql_handle]) AS txt_Blocked
WHERE  (
           tran_locks.request_mode = 'X' AND tran_locks.resource_type = 'OBJECT'
       )
...
IF EXISTS ( SELECT  Name FROM tempdb..sysobjects WHERE name LIKE '#LOCK_01_01%' )
...
SELECT 
	OBJECT_NAME(ObjId) AS [Имя объекта], 
	Mode               AS [Тип блокировки (код)],
	CASE
		WHEN Mode = 'Sch-S' THEN 'Sch-S = Блокировка стабильности схемы. Гарантирует, что элемент схемы, такой как таблица или индекс, не будет удален до тех пор, пока сеанс связи удерживает блокировку стабильности схемы на данный элемент схемы;'
	    WHEN Mode = 'Sch-М' THEN 'Sch-М = Блокировка изменения схемы. Должен поддерживаться любым сеансом связи, во время которого предполагается изменить схему данного ресурса. Гарантирует, что другие сеансы не имеют ссылок на обозначенный объект;'
 ...
 WHERE
	#LOCK_01_01.Type = 'TAB' AND #LOCK_01_01.dbid = @dbid     
...
FROM   
	sys.[sysprocesses] s
       JOIN #sysprocesses ts
            ON  s.[spid] = ts.[spid] AND s.[loginame] = ts.[loginame]
       OUTER APPLY sys.[dm_exec_sql_text](s.[sql_handle]) AS txt
WHERE  
	s.[cpu] - ts.[cpu] + s.[physical_io] - ts.[physical_io] > 500  OR  (s.[waittime] -ts.[waittime]) > 3000
GROUP BY
       txt.[text],
	   s.[spid],
	   s.[loginame],
