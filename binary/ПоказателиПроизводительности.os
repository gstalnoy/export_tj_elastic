...
// Функция предназначена для "закавычивания"
...
// Функция возвращает строку запуска клиента RAC
Функция СтрокаЗапускаКлиента()
	
	Перем ПутьКлиентаАдминистрирования;

	ПутьКлиентаАдминистрирования = ?(ЭтоWindows, ОбернутьВКавычки(мНастройки.ПутьКлиентаАдминистрирования), мНастройки.ПутьКлиентаАдминистрирования);
...
Процедура ПолучитьНастройкиИзКоманднойСтроки()
	...
	Иначе	
		...
		ИмяИндексаElasticsearch			= "app1c0"; 
		ИмяТипаElasticsearch			= "perf1c_1"; 
