...
SET @pathname = '1C_REP_EMX';
SET @pathname = @pathname + @dt;
SET @pathname = @pathname + '.bak';

SET @phys_name = '\\dev0\Development\Backup\1C.REP.EMX\' + @pathname;

PRINT 'Start create arch 1C_REP_EMX ' + @phys_name;

EXEC sp_addumpdevice 'disk', 'networkdevice', @phys_name;

BACKUP DATABASE [1C.REP.EMX] TO  
	networkdevice
	WITH COPY_ONLY, 
	NOINIT,  
	NAME = N' 1C_REP_EMX-Full Database Backup', 
	SKIP, 
	NOREWIND, 
	NOUNLOAD, 
	COMPRESSION, 
